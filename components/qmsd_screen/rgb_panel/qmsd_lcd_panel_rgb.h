...
typedef enum {
    USER_RGB_CLK_SRC_PLL160M, /*!< Select PLL160M as the source clock */
    USER_RGB_CLK_SRC_PLL240M, /*!< Select PLL160M as the source clock */
    USER_RGB_CLK_SRC_APLL,    /*!< Select APLL as the source clock */
    USER_RGB_CLK_SRC_XTAL,    /*!< Select XTAL as the source clock */
} user_rgb_clock_source_t;
...
} qmsd_lcd_rgb_timing_t;

/**
 * @brief Type of RGB LCD panel event data
 */
typedef struct {
} qmsd_lcd_rgb_panel_event_data_t;
 ...
 * @param[in] user_ctx User data, passed from `qmsd_lcd_rgb_panel_register_event_callbacks()`
 * @return Whether a high priority task has been waken up by this function
 */
typedef bool (*qmsd_lcd_rgb_panel_vsync_cb_t)(esp_lcd_panel_handle_t panel, const qmsd_lcd_rgb_panel_event_data_t *edata, void *user_ctx);
 ...
 * @param[in] user_ctx Opaque pointer that was passed from `qmsd_lcd_rgb_panel_register_event_callbacks()`
 * @return Whether a high priority task has been waken up by this function
 */
typedef bool (*qmsd_lcd_rgb_panel_bounce_buf_fill_cb_t)(esp_lcd_panel_handle_t panel, void *bounce_buf, int pos_px, int len_bytes, void *user_ctx);
...
typedef struct {
    ...
    qmsd_lcd_rgb_panel_bounce_buf_fill_cb_t on_bounce_empty; /*!< Bounce buffer empty callback. */
} qmsd_lcd_rgb_panel_event_callbacks_t;

/**
 * @brief LCD RGB panel configuration structure
 */
typedef struct {
    user_rgb_clock_source_t clk_src;   /*!< Clock source for the RGB LCD peripheral */
    qmsd_lcd_rgb_timing_t timings; /*!< RGB timing parameters, including the screen resolution */
 ...
 *
 * @note The PCLK frequency is set in the `qmsd_lcd_rgb_timing_t` and gets configured during LCD panel initialization.
